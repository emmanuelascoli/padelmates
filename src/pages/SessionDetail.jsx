    ...
    <div className="max-w-lg mx-auto space-y-5">
      ...
      <div className="card">
        ...
        <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100 flex-wrap">
          {canJoin && (
            <button onClick={handleJoin} disabled={actionLoading} className="btn-primary flex-1 text-sm">
              {actionLoading ? '...' : "S'inscrire"}
            </button>
          )}
          {canWaitlist && (
            <button onClick={handleJoinWaitlist} disabled={actionLoading} className="btn-secondary flex-1 text-sm">
              {actionLoading ? '...' : '⏳ Liste d\'attente'}
            </button>
          )}
          {isOnWaitlist && (
            <button onClick={handleLeaveWaitlist} disabled={actionLoading} className="btn-secondary flex-1 text-sm text-orange-600 border-orange-200">
              {actionLoading ? '...' : 'Quitter la liste d\'attente'}
            </button>
          )}
          {isParticipant && !isOrganizer && !isPastSession && session.status !== 'cancelled' && (
            <button onClick={handleLeave} disabled={actionLoading} className="btn-danger flex-1 text-sm">
              {actionLoading ? '...' : 'Se désinscrire'}
            </button>
          )}
          {isOrganizer && !isPastSession && session.status !== 'cancelled' && (
            <button onClick={handleCancelSession} disabled={actionLoading} className="btn-secondary text-sm text-red-600 border-red-200">
              Annuler
            </button>
          )}
        </div>
              ...
              <div key={p.id} className="card">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    ...
                    {session.cost_per_player > 0 && p.profiles?.phone && (
                      <span className="text-xs text-gray-400">📱 {p.profiles.phone}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {session.cost_per_player > 0 && participants.length > 0 && (
            <div className="card bg-blue-50 border-blue-100">
              ...
              <p className="text-sm text-blue-700">
                ...
                {session.organizer?.phone && <> — <strong>{session.organizer.phone}</strong> (Twint / Revolut)</>}.
              </p>
              {isOrganizer && (
                <p className="text-xs text-blue-500 mt-1">Clique sur le badge pour confirmer les paiements reçus.</p>
              )}
            </div>
          )}
